...
public class AtAllSpider implements Runnable {

	volatile boolean isRunning = true;
	private ThreadPoolExecutor threadPool;
	BlockingQueue<String> queue;

	public AtAllSpider(BlockingQueue<String> queue) {
		this.queue = queue;
		this.init();
	}
	...
	public void run() {
		while (this.isRunning) {
			try {
				...
				String catalog = "file/datacollection/" + course + "/" + keyword;// 得到课程目录
//				String catalog = "file/datacollection/spidertest/" + keyword;// 得到课程目录
				...
				for (int i = 0; i < s.size(); i++) {
					...
					if(authorFile.exists()){
						...
						for (int m = 0; m < authors.size(); m++) {// HttpClient爬取
//						for (int m = 0; m < authors.size()-3; m++) {// 多余三个作者（Mywork工程爬取的问题页面）
							...
							if (b.size() == 0) {
								...
								if (urls.startsWith("http://")) {
									// 获取到问题网页url，扔给线程池处理
									AtAllProcessor pro = new AtAllProcessor(keyword, atName, urls);
